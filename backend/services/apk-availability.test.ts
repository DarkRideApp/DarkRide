...
/**
 * Normalise a path argument to forward slashes so these fixtures can express
 * expectations in POSIX form. The code under test joins paths natively, so on
 * Windows the raw argument is backslash-separated and a POSIX `endsWith` never
 * matched -- every artifact looked absent and the fallback appeared broken.
 */
function posix(p: unknown): string {
