...
- **No in-app rate limiting.** Static UI assets (`/ui/*`) and plugin file serving (`/v1/files/:namespace/*`) accept unbounded request rates. In the default single-tenant deployment this only affects your own server. If you expose DarkRide on a non-trusted network, terminate at a reverse proxy that adds per-IP rate limits — this is the recommended deployment anyway, see the binding-to-non-localhost checklist above. The internal auth layer doesn't throttle by itself.
- **User-supplied regex in search/filter endpoints.** Traffic search, hostname/path filters, analysis-page search, and the API catalogue accept user-typed regular expressions and compile them with `new RegExp()`. A catastrophic-backtracking pattern (`(a+)+b` and similar) can pin a worker on a single request — i.e. ReDoS against your own server. Synchronous regex execution in Node has no native timeout. Mitigation: don't paste regexes you don't trust. Single-tenant impact only.
