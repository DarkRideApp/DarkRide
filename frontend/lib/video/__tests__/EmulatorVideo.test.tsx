...
// captures the props EmulatorVideo hands it (each render) and lets us drive its
...
// Class mock so refs resolve to an instance exposing sendKey (for the ref
// passthrough test). lastEmulator tracks the most recent instance.
...
import { EmulatorVideo, type EmulatorVideoHandle } from '../EmulatorVideo';
...
describe('EmulatorVideo', () => {
  ...
  it('defaults to the webrtc engine with a same-origin uri built from grpcWebPath', () => {
    render(<EmulatorVideo serial="localhost:32771" grpcWebPath="/v1/devices/localhost%3A32771/grpc" />);
    const props = latest();
  ...
  it('forces the png engine when initialEngine="png"', () => {
    render(<EmulatorVideo serial="x" grpcWebPath="/v1/devices/x/grpc" initialEngine="png" />);
    expect(latest().view).toBe('png');
  });

  it('fires onReady when the session reaches "connected"', async () => {
    const onReady = vi.fn();
    render(<EmulatorVideo serial="x" grpcWebPath="/v1/devices/x/grpc" onReady={onReady} />);
  ...
  it('falls back from webrtc to png on a webrtc-engine error (not a fatal onError)', async () => {
    const onError = vi.fn();
    render(<EmulatorVideo serial="x" grpcWebPath="/v1/devices/x/grpc" initialEngine="webrtc" onError={onError} />);
  ...
  it('falls back to png if webrtc does not connect within the timeout', async () => {
    vi.useFakeTimers();
    render(<EmulatorVideo serial="x" grpcWebPath="/v1/devices/x/grpc" initialEngine="webrtc" />);
    expect(latest().view).toBe('webrtc');
  ...
  it('degrades webrtc → png only after the disconnect grace window (not on a blip)', () => {
    vi.useFakeTimers();
    render(<EmulatorVideo serial="x" grpcWebPath="/v1/devices/x/grpc" />);
    expect(latest().view).toBe('webrtc');
    act(() => latest().onStateChange('connected'));
  ...
  it('exposes a ref.sendKey that delegates to the inner Emulator', () => {
    const ref = React.createRef<EmulatorVideoHandle>();
    render(<EmulatorVideo ref={ref} serial="x" grpcWebPath="/v1/devices/x/grpc" />);
    act(() => { ref.current?.sendKey('GoHome'); });
    expect(lastEmulator.sendKey).toHaveBeenCalledWith('GoHome');
    act(() => { ref.current?.sendKey('p'); });
